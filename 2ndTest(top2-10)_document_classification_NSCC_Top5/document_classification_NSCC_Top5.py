...
t0 = time()
my_stop_words = [np.unicode(x.strip(), 'utf-8') for x in open('kor_stop_word.txt', 'r').read().split('\n')]
...
def apk_per_class(actual, predicted, k=5):
	"""
    Computes the average precision at k.
    This function computes the average prescision at k between two lists of
	...
	for cl in class_score.keys():
		...
		print ("\t", cl, "Acc.:", avg, "Correct:", class_score[cl][0], "Tested:", class_score[cl][1])
		avg_acc +=avg
...
for penalty in ["l2"]: #, "l1"]:
    ...
    # Train SGD model
    results.append(benchmark(SGDClassifier(loss='log', alpha=.0001, n_iter=50, penalty=penalty)))
